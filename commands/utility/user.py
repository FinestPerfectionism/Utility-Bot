...
class UserCommands(commands.Cog):
    ...
    def resolve_timezone(self, tz_str: str) -> pytz.BaseTzInfo | list[str] | None:
        abbrev_map = {
            ...
            # Antarctic (I wonder who would ever be using these)
        ...
        if flags.s is not None:
            ...
            if not flags.tz:
                await send_custom_message(
                    ...
                    msg_type = "warning",
                    title    = "set timezone",
                    subtitle = "You must provide a timezone with `/tz`. Example: `.ti /s @user /tz EST`",
                    footer   = "Bad argument",
                )
                return
            ...
            if result is None:
                await send_custom_message(
                    ...
                    msg_type = "warning",
                    title    = "set timezone",
                    subtitle = f"Unknown timezone `{tz_str}`.",
                    footer   = "Bad argument",
                )
                return
            ...
            if isinstance(result, list):
                ...
                _ = await ctx.send(content = view.get_page_content(), view = view)
                return
            ...
            if target.bot:
                await send_custom_message(
                    ...
                    msg_type = "warning",
                    title    = "set timezone",
                    subtitle = f"**{target.display_name}** is a bot and cannot have a timezone.",
                    footer   = "Bad argument",
                )
                return
            ...
            await send_custom_message(
                ctx,
                msg_type = "success",
                title    = f"timezone for **{target.display_name}** set to **{abbr}**",
            )
            return
        ...
        if user is not None and flags.s is None:
            ...
            if user.bot:
                await send_custom_message(
                    ...
                    msg_type = "warning",
                    title    = "view timezone",
                    subtitle = f"**{user.display_name}** is a bot and doesn't have a timezone.",
                    footer   = "Bad argument",
                )
                return
            ...
            for member in ctx.guild.members:
                ...
                if len(matches) == 1:
                    ...
                    await send_custom_message(
                        ...
                        msg_type = "warning",
                        title    = "view timezone",
                        subtitle = "No users matched that name.",
                        footer   = "Bad argument",
                    )
                    return
            ...
            if uid not in timezones:
                await send_custom_message(
                    ...
                    title    = f"**{target_user.display_name}** hasn't set a timezone yet",
                )
                return
            ...
            offset   = self.format_offset(cast("discord.Member", ctx.author), target_user, timezones)
            abbr     = now.strftime("%Z")
        ...
        if flags.at is not None:
            ...
            if not tz_str:
                await send_custom_message(
                    ...
                    msg_type = "warning",
                    title    = "view timezone",
                    subtitle = "You must provide a timezone. Example: `.ti /@ PDT`",
                    footer   = "Bad argument",
                )
                return
            ...
            if isinstance(result, list):
                ...
                if 0 <= index < len(result):
                    ...
                    abbr      = now.strftime("%Z")
                    _ = await ctx.send(f"Current time in **{abbr}**: `{formatted}`")
                return
