...
def require_role(role_id : int) -> Callable[[T], T]:
...
def directors_only() -> Callable[[T], T]:
    return require_role(DIRECTORS_ROLE_ID)

def mod_and_admin_only() -> Callable[[T], T]:
...
def committee_only() -> Callable[[T], T]:
...
def has_director_role() -> Callable[[T], T]:
